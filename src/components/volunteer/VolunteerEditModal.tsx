...
export const VolunteerEditModal = ({ 
  ...
  const [formData, setFormData] = useState({
    ...
    volunteer_date: volunteer.volunteer_date ? new Date(volunteer.volunteer_date).toISOString().slice(0, 16) : "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updateData = {
        ...
        volunteer_date: formData.volunteer_date ? new Date(formData.volunteer_date).toISOString() : null,
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="volunteer_date">Date & Time *</Label>
              <Input
                id="volunteer_date"
                type="datetime-local"
                value={formData.volunteer_date}
                onChange={(e) => handleInputChange("volunteer_date", e.target.value)}
                required
              />
            </div>
          ...
          <div className="flex gap-3 pt-4">
            <Button
              type="submit"
              disabled={isSubmitting || !formData.title || !formData.volunteer_date}
              className="flex-1"
            >
