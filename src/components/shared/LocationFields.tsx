...
const compose = (address: string, city: string, state: string) =>
  [address.trim(), city.trim(), state.trim()].filter(Boolean).join(", ");
...
export const LocationFields = ({ value, onChange, required, idPrefix = "location" }: LocationFieldsProps) => {
  ...
  useEffect(() => {
    if (compose(parts.address, parts.city, parts.state) !== (value || "").trim()) {
      setParts(parse(value));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [value]);

  const update = (field: "address" | "city" | "state", fieldValue: string) => {
    const next = { ...parts, [field]: fieldValue };
    setParts(next);
    onChange(compose(next.address, next.city, next.state));
  };
