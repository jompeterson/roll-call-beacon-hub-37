// Officially ends an event: records accomplishments, funds raised and photos,
// then emails everyone who showed interest (registered users + guests) a thank-you summary.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const completionImages = rawImages
      ...
      .slice(0, 10);
    const parsedFunds = Number(body?.fundsRaised);
    const fundsRaised =
      body?.fundsRaised !== null &&
      body?.fundsRaised !== undefined &&
      Number.isFinite(parsedFunds) &&
      parsedFunds >= 0 &&
      parsedFunds <= 100000000
        ? parsedFunds
        : null;

    if (!sessionToken || !eventId || accomplishments.length === 0) {
      return new Response(
        JSON.stringify({ error: "sessionToken, eventId and at least one accomplishment are required" }),
    ...
    if (event.is_ended) {
      return new Response(JSON.stringify({ error: "This event has already been ended" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const userIds = [...new Set((rsvps || []).map((s: any) => s.user_id).filter(Boolean))];
    ...
    const photosHtml = completionImages.length > 0
      ? `<div style="margin: 0 0 20px;">${completionImages
          .map(
            (url: string) =>
              `<img src="${escapeHtml(url)}" alt="Photo from the event" width="240" style="width: 240px; max-width: 100%; border-radius: 6px; margin: 0 8px 8px 0;" />`,
          )
          .join("")}</div>`
      : "";
    const fundsFormatted = fundsRaised !== null
      ? fundsRaised.toLocaleString("en-US", { style: "currency", currency: "USD" })
      : null;
    const fundsHtml = fundsFormatted
      ? `<p style="font-size: 15px; line-height: 1.6; color: #333; margin: 0 0 20px;"><strong>Total raised:</strong> ${fundsFormatted}</p>`
      : "";
    const fundsText = fundsFormatted ? `\n\nTotal raised: ${fundsFormatted}` : "";
    const subject = `Thank you for joining us: ${event.title}`;
      ...
      const html = `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Arial, sans-serif; max-width: 560px; margin: 0 auto; padding: 24px; color: #1a1a1a;">
          ...
          <p style="font-size: 15px; line-height: 1.6; color: #333; margin: 0 0 16px;">
            Thank you for being part of <strong>${escapeHtml(event.title)}</strong>. Here is what we accomplished together:
          </p>
          ...
          <p style="font-size: 13px; color: #666; margin: 24px 0 0;">
            You received this email because you showed interest in this event.
          </p>
        </div>
      `;
      return {
        ...
        subject,
        html,
        text: `${firstName ? `Hi ${firstName},` : "Hello,"}\n\nThank you for being part of ${event.title}. Here is what we accomplished together:\n\n${listText}${fundsText}\n\nWe truly appreciate your support.`,
      };
    });
    ...
    if (!resp.ok) {
      ...
      return new Response(
        JSON.stringify({ error: "Event ended but emails failed to send", status: resp.status, details: errorBody }),
